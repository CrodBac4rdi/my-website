# Datenbank-Dokumentation (Supabase)

Diese Dokumentation beschreibt die von der App erwartete Datenbank-Struktur. Da wir auf TMDB gewechselt sind, dienen die IDs in der Datenbank als Referenz auf TMDB-Einträge.

## Tabellen

### 1. `profiles`
Speichert Benutzer-spezifische Informationen.
...
### 2. `media`
Cache-Tabelle für Medien-Metadaten, um Joins zu ermöglichen.
- `id`: Integer (Primary Key, **TMDB ID**)
- `title`: Text
- `type`: Text ('tv' oder 'movie')
- `cover_url`: Text
...
- `status`: Text ('plan_to_watch', 'watching', 'completed', 'dropped')
- `rating`: Integer (Persönliche Bewertung 1-10)
- `created_at`: Timestamp

### 4. `reviews`
Speichert Community-Feedback.
- `id`: UUID (Primary Key)
- `user_id`: UUID (Foreign Key auf `profiles.id`)
...
## SQL-Setup (für Supabase SQL Editor)

Kopiere diesen gesamten Block in den SQL Editor von Supabase und klicke auf "Run".
...
**Wichtig:** Bitte stelle sicher, dass die Row Level Security (RLS) Policies in Supabase entsprechend konfiguriert sind, damit Nutzer nur ihre eigenen Daten bearbeiten können.
